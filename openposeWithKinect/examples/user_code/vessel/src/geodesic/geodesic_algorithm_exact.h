...
namespace geodesic {
    ...
    inline bool GeodesicAlgorithmExact::construct_submesh(Mesh* sub_mesh, size_t source_idx, fScalar radius)
    {
        ...
        Vec3 srcp = ori_mesh->point(ori_mesh->vertex_handle(source_idx));//存入该顶点位置到srcp
        std::vector<bool> visited(ori_mesh->n_vertices(), false);//构建n_s的布尔向量
        std::vector<bool> added_face(ori_mesh->n_faces(), false);//构建n_f的布尔向量
        SubVidxfromMesh.resize(ori_mesh->n_vertices());//构建n_s维向量
        SubVidxfromMesh.setConstant(-1);
        MeshVidxfromSub.clear();//std::vector<int>
        visited[source_idx] = true;//设置该点为已访问
        while(!vertexlist.empty())//当顶点队列不为空时执行
        {
            size_t vidx = vertexlist.front();
            vertexlist.pop();//先进先出
            OpenMesh::VertexHandle vh = ori_mesh->vertex_handle(vidx);//存入该点的句柄
            Vec3 vp = ori_mesh->point(vh);//vp为位置
            if((srcp - vp).norm() < radius)//当传入的点位置与顶点序列头部点位置小于取样半径时执行
            {
				// std::cout<<"submesh beginnnnnnnnnnnnn"<<std::endl;
                vertex_pointer new_v = sub_mesh->add_vertex(vp);//将vp该点加入submesh
                SubVidxfromMesh[vh.idx()] = new_v.idx();//记录下在新mesh上的该点id
                MeshVidxfromSub.push_back(vh.idx());//记录下在原mesh上的该点id
                for(auto vv_it = ori_mesh->vv_begin(vh); vv_it != ori_mesh->vv_end(vh); vv_it++)//vh的相邻顶点
                {
                    if(!visited[vv_it->idx()])//如果没有访问过则执行
                    {
                        vertexlist.push(vv_it->idx());//将vh相邻的点存入vertexlist
                        visited[vv_it->idx()] = true;//标记已访问过该点
                    }
                }
                for(auto vf_it = ori_mesh->vf_begin(vh); vf_it != ori_mesh->vf_end(vh); vf_it++)//vh的相邻面
                {
                    halfedge_handle hf = ori_mesh->halfedge_handle(*vf_it);
                    if(!added_face[vf_it->idx()])
                    {
                        vertex_pointer vh = ori_mesh->from_vertex_handle(hf);//记录该边的起始点
                        vertex_pointer nextv = ori_mesh->to_vertex_handle(hf);//记录该边的终点
                            ...
                            && SubVidxfromMesh[thirdv.idx()] >= 0)
                        {
                            ...
                            sub_mesh->add_face(vertices);//将新的面加入到submesh中
                            added_face[vf_it->idx()] = true;
                        }
                    }
                }
				// std::cout<<"submesh enddddddddddddddddd"<<std::endl;
            }
        }
