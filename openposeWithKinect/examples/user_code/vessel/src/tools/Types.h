...
typedef float fScalar;//float
#else
typedef double Scalar;
#endif

typedef Eigen::SparseMatrix<fScalar, Eigen::ColMajor> ColMajorSparseMatrix;
typedef Eigen::SparseMatrix<fScalar, Eigen::RowMajor> RowMajorSparseMatrix;
...
using MatrixT = Eigen::Matrix<fScalar, Rows, Cols, Options>; ///< A typedef of the dense matrix of Eigen.
...
typedef Eigen::Matrix<fScalar, 12, 12, 0, 12, 12> EigenMatrix12;
typedef Eigen::Transform<fScalar, 3, Eigen::Affine> Affine3;


// eigen quaternions
typedef Eigen::AngleAxis<fScalar> EigenAngleAxis;
typedef Eigen::Quaternion<fScalar, Eigen::DontAlign> EigenQuaternion;
...
struct TriTraits : public OpenMesh::DefaultTraits {
    ...
    VertexTraits
    {
        ...
        {};
    public:
        fScalar geodesic_distance;
        VertexState state;
        OpenMesh::FaceHandle incident_face;
        fScalar incident_point;
...
public:
    ...
    Matrix3333 operator* (fScalar multi);
    friend Matrix3333 operator* (fScalar multi1, Matrix3333& multi2);
...
public:
    ...
    Matrix2222 operator* (fScalar multi);
    friend Matrix2222 operator* (fScalar multi1, Matrix2222& multi2);
